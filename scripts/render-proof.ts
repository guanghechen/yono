...
const derivative = process.argv[2] ?? fileURLToPath(new URL(`build/${design.postScriptName}.ttf`, root))
...
const samples: readonly (readonly [number, number, string])[] = [
  ...
  [1060, 38, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'],
...
drawText(args, ui, 25, 80, 126, '同字号、同基线对比 · 中文行楷 · 完整 ASCII 白板手写', '#758194')
...
drawText(args, ui, 23, 80, 1300, `当前覆盖 ${current.cmap.size.toLocaleString('en-US')} 个码点；全部目标汉字已重建，其他 Maple 符号及图标仍待补齐。`, '#758194')
